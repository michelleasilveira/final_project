...
In this report we build and compare predictive models for diabetes using the
**Diabetes Health Indicators** dataset
(`diabetes_binary_health_indicators_BRFSS2015`). The original outcome
variable, `Diabetes_012`, has three categories:

- `0` = no diabetes  
- `1` = prediabetes  
- `2` = diabetes  

For this project we follow the course instructions and work with a **binary**
response. We define a new variable, `Diabetes_binary`, that indicates whether
a respondent has diabetes (`1`) or not (`0`):
...
- Use **log-loss** with 5-fold cross-validation to select the best model in
  each family.
- Compare the final tuned models on the test set and select a winner.

We use the **tidymodels** framework to keep preprocessing, resampling,
tuning, and evaluation organized and reproducible.
...
Here we read the same dataset used in the EDA and perform minimal cleaning so
that the modeling file is self-contained. We:
...
*The binary outcome is imbalanced, with the “Diabetes” class representing a
smaller proportion of observations. This will influence how we interpret
...
We now split the data into a training set (70%) and a test set (30%),
stratifying on the binary outcome to preserve class proportions in each
partition.
...
Guided by the EDA, we choose a set of predictors that showed meaningful
...
We also set up 5-fold cross-validation on the training data, stratifying by
the outcome, and define a metric set centered on **log-loss** (here named
`mn_log_loss` in this version of yardstick), with accuracy and ROC AUC for
additional context.
...
We create a regular grid of hyperparameters to explore a range of tree sizes
and complexities.
...
We combine the recipe and model into a workflow and use 5-fold cross-validation
to estimate performance across the grid, focusing on log-loss.
...
We examine the best-performing tree configurations according to **mn_log_loss**
(lower is better).
...
We now select the single best set of hyperparameters and finalize the tree
workflow.
...
We fit the finalized tree model on the full training data and evaluate its
...
We now consider a **random forest** model, which fits an ensemble of decision
trees and averages their predictions. The main tuning parameters are:
...
We set up a grid for `mtry` and `min_n`. Because the number of predictors
after dummy encoding is not known ahead of time, we estimate a reasonable
range for `mtry` based on the training data.
...
We create a workflow for the random forest model and tune it using the same
5-fold cross-validation folds and log-loss metric.
...
We fit the finalized random forest on the full training data and evaluate it
on the test set.
...
rf_metrics <- logloss_metrics(
  rf_probs,
  truth    = Diabetes_binary,
  estimate = .pred_Diabetes
)
...
We now compare the **test set performance** of the tuned classification tree
and random forest models. Our primary metric is **log-loss** (reported here as
`mn_log_loss`, lower is better), but we also consider accuracy and ROC AUC.
...
*Typically we expect the random forest to outperform a single tree on log-loss
and ROC AUC due to its ensemble nature and better ability to capture complex
interactions. If that is the case here, we will choose the random forest as
our final model for deployment in the API.*
...
- Created a binary diabetes outcome from the original three-level `Diabetes_012`
  variable.
- Split the data into stratified training and test sets.
- Built a preprocessing recipe that handles missing values and encodes
...
Based on the test set results, we select the model with the **lowest log-loss**
(and typically higher ROC AUC) as our final model. This chosen model will be
refit on the full dataset and exposed via a **plumber API** in the next stage
of the project.
